# WP Nginx PHP MySQL WP CLI

Docker running Nginx, PHP-FPM, Composer, MySQL, PHPMyAdmin, WP CLI , Cron.
...
To install [Git](https://github.com/Aharonyan/wp-docker-nginx-php-mysql), download it and install following the instructions :

```sh
git clone git@github.com:Aharonyan/wp-docker-nginx-php-mysql.git
```
...
2. Start the application from root folder:
...
### PHP shell access
```sh
sudo docker exec -it www-php-wp_cli bash
```
...
```sh
sudo docker exec -it www-mysql bash
```
